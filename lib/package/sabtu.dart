...
class Sabtupage extends StatefulWidget {
  const Sabtupage({Key? key}) : super(key: key);

  @override
  State<Sabtupage> createState() => _JadwalShalatState();
}

class _JadwalShalatState extends State<Sabtupage> {
  String subuh = '';
  String zuhur = '';
  String ashar = '';
  String magrib = '';
  ...
  void initState() {
    ...
    // Set the selected date to the nearest Saturday
    selectedDate = _getNearestSaturday(selectedDate);
    getPrayerTimes(selectedDate);
  }

  // Function to get the nearest Saturday
  DateTime _getNearestSaturday(DateTime date) {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != selectedDate) {
      setState(() {
        selectedDate = picked;
        // Ensure selected date is a Saturday
        selectedDate = _getNearestSaturday(selectedDate);
      });
      getPrayerTimes(selectedDate);
    }
      ...
      body: ListView(children: [
        Column(
          ...
          children: [
            ...
            Divider(),
            // ElevatedButton(
            //   onPressed: () => InputJadwal(),
            //   child: Text('Input Jadwal'),
            // ),
          ],
        ),
      ]),
